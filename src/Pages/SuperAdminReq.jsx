 ...
 *  - (Optional) GET /api/super-admin/role — to verify user is admin; not required here because
...
export default function AdminSignupRequests() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const url = `${API_BASE}/api/super-admin/admin/signup-requests?status=${encodeURIComponent(
        status
      )}`.replace(/\/+\/\//g, "/");
  ...
  const doAction = async (id, action) => {
    try {
      setBanner(null);
      const url = `${API_BASE}/api/super-admin/admin/signup-requests/${id}/${action}`.replace(/\/+\/\//g, "/");
